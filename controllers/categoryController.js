...
exports.category_detail = asyncHandler(async (req, res, next) => {
  ...
  try {
    ...
    allPartsInCategory = await Part.find({ category: req.params.id }).exec();
  } catch {
    thisCategory = null;
    allPartsInCategory = [];
  }
...
exports.category_create_post = [
  body('category_name', 'Category must have a name.')
    ...
    .escape()
    .withMessage('Category name cannot be longer than 32 characters.'),
  
  body('category_description', 'Category must have a description.')
    .trim()
    .isLength({ min: 1, max: 512 })
    .escape()
    .withMessage('Category description cannot be longer than 512 characters.'),
